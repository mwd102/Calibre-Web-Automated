...
def delete_user_rows(session, user_id):
    ...
    shelf_ids = session.query(ub.Shelf.id).filter(ub.Shelf.user_id == user_id).subquery()
    ...
    magic_ids = session.query(ub.MagicShelf.id).filter(ub.MagicShelf.user_id == user_id).subquery()
